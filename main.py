...
async def nlist(chat_id: int, page: int = 1):
    try:
        ...
        sql.execute(f"SELECT name FROM sqlite_master WHERE type='table' AND name='nicks_{chat_id}'")
        if not sql.fetchone():
            return ["В этом чате еще нет ников!"]

        # Получаем все ники из таблицы
        sql.execute(f"SELECT user_id, nick FROM nicks_{chat_id} ORDER BY user_id")
        fetch = sql.fetchall()
        ...
        with open("config.json", "r") as f:
            config = json.load(f)
        max_nicks = config.get('nicks_max', 10)  # Значение по умолчанию 10

        # Рассчитываем диапазон для пагинации
        total_pages = (len(fetch) + max_nicks - 1) // max_nicks
        page = max(1, min(page, total_pages))  # Защита от некорректной страницы
        
        start_idx = (page - 1) * max_nicks
        end_idx = start_idx + max_nicks
        current_nicks = fetch[start_idx:end_idx]

        # Получаем информацию о пользователях
        user_ids = [str(user_id) for user_id, _ in current_nicks]
        users_info = await bot.api.users.get(user_ids=user_ids)

        # Формируем список ников
        nicks = []
        for idx, (user_id, nick) in enumerate(current_nicks, start=start_idx + 1):
            user = next((u for u in users_info if u.id == user_id), None)
            if user:
                nicks.append(f"{idx}) @id{user_id} ({user.first_name} {user.last_name}) - {nick}")
            else:
                nicks.append(f"{idx}) @id{user_id} (неизвестный) - {nick}")
        ...
        return ["Произошла ошибка при получении списка ников"]
...
async def on_chat_message(message: Message):
    ...
    if command_identifier in bot_identifiers:
        ...
        if message.reply_message and message.reply_message.from_id < 0:
            ...
            if command in ['stats', 'стата', 'статистика', 'stata', 'statistic']:
                ...
                try:
                    ...
                    user = user_id  # По умолчанию - автор команды
                    ...
                    if message.reply_message:
                        ...
                        possible_id = await getID(arguments[1])
                        if possible_id:
                            user = possible_id

                    
                    # Получаем данные
                    info = (await bot.api.users.get(user_ids=user))[0]
                    reg_data = await get_registration_date(user)
                    role = await get_role(user, chat_id)
                    warns = await get_warns(user, chat_id)
                    ...
                    # Получаем ник
                    nick = await get_user_name(user, chat_id) if await is_nick(user, chat_id) else "Нет"
                    ...
                    # Формируем ответ
                    response = (
                        f"@id{user_id} ({await get_user_name(user_id, chat_id)}), статистика @id{user} (пользователя):\n"
                        f"📊 Имя и фамилия: {info.first_name} {info.last_name}\n"
                        f"📅 Дата регистрации: {reg_data}\n"
                        f"🏷️ Ник: {nick}\n"
                        f"⚖️ Роль: {roles.get(role, 'Неизвестно')}\n"
                        f"⚠️ Всего предупреждений: {warns}/3\n"
                        f"✉️ Всего сообщений: {messages['count']}\n"
                        f"🕒 Последнее сообщение: {messages['last']}"
                    )

                    # Редактируем исходное сообщение с командой
                    x = await bot.api.messages.get_by_conversation_message_id(
        ...
        if command in ['sban', 'сбан']:
            ...
            # Получаем название сетки из аргументов
            if len(arguments) < 2:
                await message.reply("Укажите название сетки и пользователя!\nПример: /sban основная @user123 Причина", disable_mentions=1)
                return True

            setka_name = arguments[1].lower()
            
            # Преобразуем название в числовой ID
            setka_value = 0
            if setka_name != 'нет':
                for i, char in enumerate(setka_name[:4]):
                    setka_value += (ord(char) - ord('а') + 1) * (30 ** i)
                setka_value = min(setka_value, 2000)
            ...
            if message.reply_message:
                user = message.reply_message.from_id
                reason_start = 1
            elif len(arguments) >= 3 and await getID(arguments[2]):
                user = await getID(arguments[2])
                reason_start = 3
            else:
                await message.reply("Укажите пользователя!", disable_mentions=1)
                return True
            ...
            reason = await get_string(arguments, reason_start)
            if not reason:
                await message.reply("Укажите причину блокировки!", disable_mentions=1)
                return True

            # Получаем чаты сетки
            sql.execute("SELECT chat_id FROM chats WHERE in_pull = ?", (setka_value,))
            pull_chats = [row[0] for row in sql.fetchall()]

            if not pull_chats:
                await message.reply(f"Сетка бесед '{setka_name}' не найдена или пуста", disable_mentions=1)
                return True

            success_bans = 0
            for chat in pull_chats:
                try:
                    await ban(user, user_id, chat, reason)
                    try:
                        await bot.api.messages.remove_chat_user(chat, user)
                    except:
                        pass
                    
                    keyboard = Keyboard(inline=True).add(
                        Callback("Снять бан", {"command": "unban", "user": user, "chatId": chat}),
                        color=KeyboardButtonColor.POSITIVE
                    )
                    
                    await bot.api.messages.send(
                        peer_id=2000000000 + chat,
                        message=f"🔴 Глобальная блокировка в сетке '{setka_name}'\n"
                            f"Администратор: @id{user_id}\n"
                            f"Заблокирован: @id{user}\n"
                            f"Причина: {reason}",
                        disable_mentions=1,
                        random_id=0,
                        keyboard=keyboard
                    ...
                    print(f"Ошибка бана в чате {chat}: {e}")

            await message.answer(
                f"🔴 @id{user_id} выполнил глобальную блокировку в сетке '{setka_name}'\n"
                f"Заблокирован: @id{user}\n"
                f"Причина: {reason}\n"
                f"Успешно: {success_bans}/{len(pull_chats)} бесед",
                disable_mentions=1
            )
            ...
            if await get_sliv(user_id, chat_id) and await get_role(user_id, chat_id) < 5:
                await roleG(user_id, chat_id, 0)
                await message.reply(
                    f"❗️ Уровень прав @id{user_id} был снят из-за подозрений в сливе беседы\n\n{await staff_zov(chat_id)}")
        ...
        if command in ['sunban', 'санбан', 'сунбан']:
            ...
            # Получаем название сетки
            if len(arguments) < 2:
                await message.reply("Укажите название сетки и пользователя!\nПример: /sunban основная @user123", disable_mentions=1)
                return True
            ...
            for chat in pull_chats:
                try:
                    await unban(user, chat)
                    success_unbans += 1
                except:
                    pass

            await message.answer(
                f"🟢 @id{user_id} выполнил глобальную разблокировку в сетке '{setka_name}'\n"
                f"Разблокирован: @id{user}\n"
                f"Успешно: {success_unbans}/{len(pull_chats)} бесед",
                disable_mentions=1
            )
        ...
        if command in ['pullinfo', 'pulli']:
            ...
            pull = await get_pull_chats(chat_id)
            pull_id = await get_pull_id(chat_id)
            
            if not pull: 
                pull_str = "Беседа не находится в связке!"
            else:
                # Преобразуем числовой ID обратно в буквенное название
                if pull_id == 0:
                    pull_name = "нет"
                else:
                    # Обратное преобразование числа в буквы (первые 4 символа)
                    temp = pull_id
                    pull_name = ""
                    for _ in range(4):
                        temp, remainder = divmod(temp, 30)
                        if remainder > 0:
                            pull_name += chr(ord('а') + remainder - 1)
                        else:
                            break
                    pull_name = pull_name[::-1]  # Разворачиваем строку
                    
                pull_str = (f"Название сетки бесед: {pull_name}\n"
                        f"Всего бесед в сетке: {len(pull)}")
            
            await message.reply(f"Информация о сетке бесед:\n{pull_str}", disable_mentions=1)
        ...
        if command in ['pull', 'setpull']:
            if await get_role(user_id, chat_id) < 5:
                await message.reply("Недостаточно прав для выполнения этой операции!", disable_mentions=1)
                return True

            if len(arguments) < 2:
                await message.reply("Пожалуйста, укажите название сетки бесед. Для удаления привязки укажите 'нет'", disable_mentions=1)
                return True

            setka_name = arguments[1].lower()
            
            # Преобразование буквенного идентификатора в числовой (по первым 4 буквам)
            if setka_name == 'нет':
                setka_value = 0
            else:
                # Создаем числовое значение из первых 4 букв названия
                setka_value = 0
                for i, char in enumerate(setka_name[:4]):
                    setka_value += (ord(char) - ord('а') + 1) * (30 ** i)  # Русские буквы
                
                # Ограничиваем максимальное значение
                setka_value = min(setka_value, 2000)
                if setka_value == 0:
                    setka_value = 1  # Минимальное значение, если название дало 0

            await set_pull(chat_id, setka_value)
            user_name = await get_user_name(user_id, chat_id)
            
            if setka_value == 0:
                await message.answer(f"Пользователь @id{user_id} ({user_name}) удалил привязку к сетке бесед", disable_mentions=1)
            else:
                await message.answer(
                    f"Пользователь @id{user_id} ({user_name}) установил привязку к сетке бесед: {setka_name}", 
                    disable_mentions=1
                )
        if command in ['news']:
        ...
        if command in ['szov', 'serverzov', 'сзов']:
            ...
            if len(arguments) < 2:
                await message.reply("Укажите название сетки и причину вызова!\nПример: /szov основная Срочное собрание", disable_mentions=1)
                return True
            ...
            # Преобразуем буквенное название в числовой ID (как в команде pull)
            setka_value = 0
            if setka_name != 'нет':
                for i, char in enumerate(setka_name[:4]):
                    setka_value += (ord(char) - ord('а') + 1) * (30 ** i)
            ...
            # Получаем чаты сетки
            sql.execute(f"SELECT chat_id FROM chats WHERE in_pull = ?", (setka_value,))
            pull_chats = [row[0] for row in sql.fetchall()]

            if not pull_chats:
                await message.reply(f"Сетка бесед '{setka_name}' не найдена или не содержит активных чатов", disable_mentions=1)
                return True

            success_count = 0
            for chat in pull_chats:
                try:
                    users = await bot.api.messages.get_conversation_members(
                        peer_id=2000000000 + chat,
                        fields=["online_info", "online"]
                    )
                    users = json.loads(users.json())
                    ...
                    for user in users["profiles"]:
                        if user['id'] != user_id and count < 100:
                            user_mentions.append(f"@id{user['id']} (🔔)")
                            count += 1

                    await bot.api.messages.send(
                        peer_id=2000000000 + chat,
                        message=f"🔔 Серверный вызов из сетки '{setka_name}'\n"
                            f"От: @id{user_id} ({await get_user_name(user_id, chat_id)})\n\n"
                            f"{''.join(user_mentions)}\n\n"
                            f"📢 Причина: {reason}",
                        random_id=0,
                        disable_mentions=1
                    )
            ...
            await message.reply(
                f"✅ Серверный вызов отправлен в {success_count} бесед сетки '{setka_name}'\n"
                f"Причина: {reason}",
                disable_mentions=1
            )
